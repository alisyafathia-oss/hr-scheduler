...
// GET  /api/slots              — list available slots (filtered by meeting type / date)
// POST /api/slots              — team head creates a slot
// POST /api/slots/book         — employee books a slot
// GET  /api/slots/availability — reads team head's Google Calendar for free windows

const { requireAuth, requireRole } = require("../../src/lib/auth");
const { readSheet, appendSheet, writeSheet, listFreeBusy, createCalendarEvent } = require("../../src/lib/google-client");
const { rowToMeeting, meetingToRow } = require("../../src/lib/schedule-engine");
const { sendEmail, bookingConfirmationEmail } = require("../../src/lib/mailer");
const { addMinutes, format, parseISO, addDays, setHours, setMinutes } = require("date-fns");

const SHEET_ID   = () => process.env.CONTRACTS_SHEET_ID;
const SLOTS_RANGE = "Slots!A2:J";
const MEET_RANGE  = "Meetings!A2:O";

// Slot row: [id, meetingId, teamHeadEmail, date, startTime, endTime, status, bookedBy, calendarEventId, createdAt]
...
exports.handler = async (event) => {
  ...
  try {
    ...
    if (method === "GET" && path === "availability") {
      ...
      const busyBlocks = await listFreeBusy(
        calendarId,
        now.toISOString(),
        until.toISOString()
      );

      // Build suggested free windows (9am–5pm, 30-min slots, skip weekends)
      const suggestions = [];
    ...
    if (method === "POST" && path === "book") {
      ...
      try {
        ...
        const event   = await createCalendarEvent(process.env.HR_CALENDAR_ID, {
