...
// POST /api/slots/delete — HR admin or owning team head deletes a slot.
...
async function handler(event) {
  ...
  try {
    ...
    if (session.role !== "hr_admin" && slot.teamHeadEmail !== (session.workEmail || session.email)) {
    ...
    // Blank out the row to soft-delete (keeps row count stable)
    const rowNum = idx + 2;
    await writeSheet(SHEET_ID(), `Slots!A${rowNum}:J${rowNum}`, [["", "", "", "", "", "", "deleted", "", "", ""]]);
